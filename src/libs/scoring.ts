...
export function detectImpossibleTravel(
  ...
  const elapsedHours =
    Math.abs(current.ts.getTime() - last.ts.getTime()) / (1000 * 60 * 60);
  if (elapsedHours === 0) return distanceKm > 0;
...
export function computeRiskDelta(
  ...
  const avg =
    history.reduce((sum, s) => sum + s.enrichment.riskScore, 0) / history.length;
